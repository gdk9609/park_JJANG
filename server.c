...
static int response_printf(const char *fmt, ...)
{
    ...
    va_end(ap);
    if (written > 0)
        response_buffer.len += (size_t)written;
    return written;
}
...
// 특수문자를 이스케이프 처리 후 JSON 문자열 형태로 버퍼에 출력
...
static const char *get_param(const char *name)
{
    ...
    while (*p)
    {
        ...
        if (!amp)
            amp = key_start + strlen(key_start);
...
static int parse_tower_id(const char *s)
{
    if (!s || !*s)
        return 0;
    if (strcmp(s, "1") == 0 || s[0] == 'A' || s[0] == 'a')
        return 1;
    if (strcmp(s, "2") == 0 || s[0] == 'B' || s[0] == 'b')
        return 2;
    if (strcmp(s, "3") == 0 || s[0] == 'C' || s[0] == 'c')
        return 3;
    return atoi(s);
}
...
static int parse_car_type(const char *s)
{
    if (!s || !*s)
        return 0;
    if (strcmp(s, "1") == 0 || strcmp(s, "경차") == 0)
        return CAR_COMPACT;
    if (strcmp(s, "2") == 0 || strcmp(s, "중형차") == 0)
        return CAR_MIDSIZE;
    if (strcmp(s, "3") == 0 || strcmp(s, "SUV") == 0 || strcmp(s, "suv") == 0)
        return CAR_SUV;
    if (strcmp(s, "4") == 0 || strcmp(s, "대형차") == 0)
        return CAR_LARGE;
    if (strcmp(s, "5") == 0 || strcmp(s, "전기차") == 0)
        return CAR_EV;
    return atoi(s);
}
...
// 타워 사용률에 따른 상태 텍스트 및 CSS 클래스명 결정
...
static void print_reservation_json_fields(const Reservation *r)
{
    ...
    printf(",\"reservationNo\":");
    json_escape_print(r->code);
    printf(",\"carNumber\":");
    json_escape_print(r->car_number);
    printf(",\"phoneNumber\":");
    json_escape_print(r->phone);
    printf(",\"carType\":");
    json_escape_print(car_type_name(r->car_type));
    printf(",\"carTypeId\":%d", r->car_type);
    printf(",\"towerId\":%d", r->tower_id);
    printf(",\"parkingTower\":");
    json_escape_print(towername);
}
...
static void action_reserve(void)
{
    ...
    if (!*tower_text)
        copy_param(tower_text, sizeof(tower_text), "tower");
    ...
    printf(",\"operatingHours\":");
    json_escape_print(DEFAULT_OPERATING_HOURS);
    printf(",");
    print_time_field("reserveTime", r.reserved_at);
    printf(",\"limitTime\":\"20분\"");
    printf(",\"deposit\":");
    json_escape_print(money);
    printf("}");
}
...
static void action_entry(void)
{
    ...
    if (!*code)
        copy_param(code, sizeof(code), "code");
    copy_param(car_number, sizeof(car_number), "carNumber");
    ...
    printf("{\"success\":true");
    printf(",\"reservationNo\":");
    json_escape_print(p.reservation_code);
    printf(",\"carNumber\":");
    json_escape_print(p.car_number);
    printf(",\"phoneNumber\":");
    json_escape_print(p.phone);
    printf(",\"carType\":");
    json_escape_print(car_type_name(p.car_type));
    printf(",\"carTypeId\":%d", p.car_type);
    printf(",\"towerId\":%d", p.tower_id);
    printf(",\"parkingTower\":");
    json_escape_print(towername);
    printf(",");
    print_time_field("entryTime", p.entry_time);
    printf("}");
}
...
static void print_status_json(int status, const Reservation *r, const ParkingRecord *p, int remaining)
{
    ...
    if (status == RES_RESERVED && r)
    {
        ...
        char rem[64];
        snprintf(rem, sizeof(rem), "%d분 %d초", remaining / 60, remaining % 60);
        printf(",\"remainingText\":");
        json_escape_print(rem);
    }
    else if (status == RES_ENTERED && p)
    {
        ...
        print_time_field("entryTime", p->entry_time);
        printf(",\"elapsedText\":");
        json_escape_print(dur);
        printf(",\"currentFee\":%d", current_fee);
        printf(",\"currentFeeText\":");
        json_escape_print(feeText);
    }
    printf("}");
}
...
static void print_payment_json(const Payment *p, int completed)
{
    ...
    json_escape_print(p->reservation_code);
    printf(",\"receiptNo\":");
    json_escape_print(p->receipt_no);
    printf(",\"carNumber\":");
    json_escape_print(p->car_number);
    printf(",\"phoneNumber\":");
    json_escape_print(p->phone);
    ...
    print_time_field("entryTime", p->entry_time);
    printf(",");
    print_time_field("exitTime", p->exit_time);
    printf(",\"durationText\":");
    json_escape_print(duration);
    ...
    format_money(p->parking_fee, money, sizeof(money));
    printf(",\"parkingFeeText\":");
    json_escape_print(money);
    format_money(p->deposit_used, money, sizeof(money));
    printf(",\"depositUsedText\":");
    json_escape_print(money);
    format_money(p->final_fee, money, sizeof(money));
    printf(",\"finalFeeText\":");
    json_escape_print(money);
    printf(",\"method\":");
    json_escape_print(p->method);
    printf("}");
}
...
static void action_fee_calc(void)
{
    ...
    json_escape_print(duration);
    printf(",\"chargedText\":");
    json_escape_print(charged_text);
    printf(",\"fee\":%d", fee);
    format_money(fee, money, sizeof(money));
    printf(",\"feeText\":");
    json_escape_print(money);
...
static void action_tower_overview(void)
{
    ...
    for (int id = 1; id <= 3; id++)
    {
        ...
        printf(",\"operatingHours\":");
        json_escape_print(t.operating_hours);
        printf(",\"status\":");
        json_escape_print(status);
        printf(",\"statusClass\":");
        json_escape_print(status_class);
        printf(",\"carTypes\":[");
        for (int type = 1; type <= CAR_TYPE_COUNT; type++)
        {
            if (type > 1)
            ...
            printf("{\"id\":%d,\"name\":", type);
            json_escape_print(car_type_name(type));
...
// (관리자) 특정 타워 전체 입차/예약 차량 상세 목록 조회
static void action_admin_tower(void)
{
    ...
    printf("\"key\":");
    json_escape_print(tower_key(tower_id));
    printf(",\"id\":%d,\"name\":", tower_id);
    json_escape_print(t.name);
    printf(",\"capacity\":%d,\"usedTotal\":%d,\"sales\":%d,\"status\":", t.total_capacity, entered + reserved, tower_sales[tower_id]);
    json_escape_print(status);
    printf(",\"statusClass\":");
    json_escape_print(status_class);
    printf("},\"entered\":[");
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        if (read_record_at(RESERVATIONS_FILE, i, &r, sizeof(r)) == 0 && r.tower_id == tower_id && r.status == RES_RESERVED)
        {
            ...
            printf(",\"limitTime\":");
            char rembuf[64];
            snprintf(rembuf, sizeof(rembuf), "%d분 %d초", rem / 60, rem % 60);
            json_escape_print(rembuf);
            printf("}");
        }
    }
    printf("]}");
...
static void action_update_entry_time(void)
{
    ...
    printf("{\"success\":true,\"reservationNo\":");
    json_escape_print(p.reservation_code);
    printf(",\"carNumber\":");
    json_escape_print(p.car_number);
    printf(",\"phoneNumber\":");
...
static void action_change_reservation(void)
{
    ...
    printf(",");
    print_time_field("reserveTime", r.reserved_at);
    printf(",\"limitTime\":\"20분\",\"deposit\":");
    json_escape_print(money);
    printf("}");
}

// 요청된 'action' 파라미터 값에 따라 알맞은 함수로 라우팅
static void run_api_action(void)
{
    ...
    if (!*action)
        ...
        action_find();
    else if (strcmp(action, "settle_preview") == 0)
        action_settle_preview();
    else if (strcmp(action, "exit") == 0)
        action_exit();
    else if (strcmp(action, "fee_calc") == 0)
        action_fee_calc();
    else if (strcmp(action, "tower_overview") == 0)
        action_tower_overview();
    else if (strcmp(action, "admin_summary") == 0)
        action_admin_summary();
    else if (strcmp(action, "admin_tower") == 0)
        action_admin_tower();
    else if (strcmp(action, "update_entry_time") == 0)
        action_update_entry_time();
    else if (strcmp(action, "cancel_reservation") == 0)
        action_cancel_reservation();
    else if (strcmp(action, "change_reservation") == 0)
        action_change_reservation();
    else
        print_error("지원하지 않는 action입니다.");
}
...
// 상태 코드, Content-Type, 바디 데이터 HTTP 응답 전송
...
// API 요청 실패 시 HTTP 400 상태와 에러 응답 전송
...
// HTTP 헤더에서 Content-Length 값 추출 후 Body 크기 반환
static int parse_content_length(const char *headers)
{
    ...
    while ((p = strcasestr(p, "Content-Length:")) != NULL)
    {
        if (p == headers || p[-1] == '\n')
        {
            ...
            if (value < 0)
                return 0;
            if (value > MAX_HTTP_REQUEST)
                return MAX_HTTP_REQUEST;
            return (int)value;
        }
        p += 15;
    }
...
// 클라이언트 소켓에서 HTTP 요청 전체를 동적 메모리에 저장
static char *read_http_request(int fd, size_t *out_len)
{
    ...
    while (len < MAX_HTTP_REQUEST)
    {
        ...
        {
            cap *= 2;
            if (cap > MAX_HTTP_REQUEST)
                cap = MAX_HTTP_REQUEST;
        ...
        if (header_end)
        {
            ...
            if (len >= header_len + (size_t)content_length)
                break;
        }
    }
...
// 클라이언트 접속 시 생성 스레드 진입점 (요청 분석 후 정적파일/API 처리 분기)
static void *client_thread(void *arg)
{
             ...
             (strcmp(method, "POST") == 0 || strcmp(method, "GET") == 0))
    {
        ...
        const char *body = "";
        if (strcmp(method, "POST") == 0 && header_end)
            body = header_end;
...
/*
 * 메인 함수
 * - 소켓 생성 및 포트(기본 8080) 바인딩
 * - 무한 루프를 돌며 클라이언트의 접속(accept) 대기
 * - 접속 시 pthread_create로 새로운 client_thread 생성
 */
